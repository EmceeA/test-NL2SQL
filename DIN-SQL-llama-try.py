...
#                 {"role": "user", "content": prompt}
#             ],
#             max_tokens=5000,  # Attempt to set max_tokens to 5000
#             temperature=0.7,   # Adjust temperature for more deterministic or varied outputs
#             top_p=1.0          # Adjust top_p if necessary
#         )
...
#         # Check the output format of 'response' and parse it correctly
#         if 'choices' in response and len(response['choices']) > 0:
#             # Assuming the text of the first choice is the desired output
#             output_text = response['choices'][0]['text']
#             return output_text.strip()
#         else:
#             print("No completion found.")
...
#         # This is a basic example of splitting by sentences; adjust according to your data structure
#         parts = prompt.split('. ')
#         responses = []
...
#             else:
#                 print("No completion found for part:", part)
...
if sys.argv[1] == "--dataset" and sys.argv[3] == "--output":
    # Assuming sys.argv[2] is 'data/' and ensuring paths are correctly joined
    import os
...
response = llama_generation(schema_linking_prompt)
    ...
    try:
        for index, row in val_df.iterrows():
            ...
            while classification is None:
                try:
                    classification = llama_generation(classification_prompt_maker(row['question'], row['db_id'], schema_links[1:]))
            ...
            if '"EASY"' in predicted_class:
                ...
                while SQL is None:
                    try:
                        SQL = llama_generation(medium_prompt_maker(row['question'], row['db_id'], schema_links))
                        SQL = SQL.split("SQL: ")[1]
                    except:
                        time.sleep(3)
                ...
                while SQL is None:
                    try:
                        SQL = llama_generation(hard_prompt_maker(row['question'], row['db_id'], schema_links, sub_questions))
                        SQL = SQL.split("SQL: ")[1]
                    except:
                        time.sleep(3)
            ...
            while debugged_SQL is None:
                try:
                    debugged_SQL = llama_generation(debuger(row['question'], row['db_id'], SQL)).replace("\n", " ")
                except:
                    time.sleep(3)
